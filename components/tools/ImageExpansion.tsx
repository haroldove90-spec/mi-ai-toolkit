...
const ImageExpansion: React.FC<ImageExpansionProps> = ({ tool, onBack }) => {
    ...
    const handleGenerate = useCallback(async () => {
        ...
        setResultImage(null);
        try {
            const prompt = "Expand this image, filling in the missing areas coherently based on the existing content. Maintain the original style. This is also known as outpainting.";
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="mt-6 mb-8 p-4 bg-gray-800 border border-gray-700 rounded-lg text-center">
                <p className="text-sm text-gray-300">
                    <span className="font-semibold text-purple-400">¿Cómo funciona?</span> Sube una imagen que quieras ampliar. La IA rellenará de forma inteligente las áreas vacías alrededor de la imagen original, manteniendo el estilo y la coherencia (outpainting).
                </p>
            </div>
            ...
            <div className="bg-gray-800/50 p-6 rounded-lg text-center">
                <div className="w-full p-4 border-2 border-dashed border-gray-600 rounded-lg">
                    ...
                    <label htmlFor="image-upload" className={`cursor-pointer ${isLoading ? 'cursor-not-allowed' : ''}`}>
                        {imagePreview ? (
                            <p>Imagen seleccionada. ¡Lista para expandir!</p>
                        ) : (
                            <p>Click to upload an image to expand</p>
                        )}
                    </label>
                </div>
                 <button
                ...
                >
                    {isLoading && <Spinner />}
                    {isLoading ? 'Expandiendo Imagen...' : 'Expandir Imagen'}
                </button>
            </div>
                 ...
                 <div>
                    <h3 className="text-xl font-semibold mb-4 text-center">Resultado</h3>
                     <div className="w-full aspect-square bg-gray-700 rounded-lg shadow-lg p-2">
                        ...
                        {resultImage && <img src={resultImage} alt="Expanded image" className="w-full h-full object-contain" />}
                    </div>
                </div>
            </div>
        </div>

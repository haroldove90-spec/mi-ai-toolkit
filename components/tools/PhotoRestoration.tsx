...
const PhotoRestoration: React.FC<PhotoRestorationProps> = ({ tool, onBack }) => {
    ...
    const handleGenerate = useCallback(async () => {
        ...
        setResultImage(null);
        try {
            const prompt = "Restore this old photograph. Remove scratches, fix tears, and improve the details and color balance.";
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="mt-6 mb-8 p-4 bg-gray-800 border border-gray-700 rounded-lg text-center">
                <p className="text-sm text-gray-300">
                    <span className="font-semibold text-purple-400">¿Cómo funciona?</span> Sube una fotografía antigua, dañada o con arañazos. La IA la restaurará, mejorando la calidad, eliminando imperfecciones y corrigiendo los colores.
                </p>
            </div>
            ...
            <div className="bg-gray-800/50 p-6 rounded-lg text-center">
                <div className="w-full p-4 border-2 border-dashed border-gray-600 rounded-lg">
                    ...
                    <label htmlFor="image-upload" className={`cursor-pointer ${isLoading ? 'cursor-not-allowed' : ''}`}>
                        {imagePreview ? (
                            <p>Foto seleccionada. ¡Lista para restaurar!</p>
                        ) : (
                            <p>Click to upload an old photo to restore</p>
                        )}
                    </label>
                </div>
                 <button
                ...
                >
                    {isLoading && <Spinner />}
                    {isLoading ? 'Restaurando Foto...' : 'Restaurar Foto'}
                </button>
            </div>
            ...
            <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                <div>
                 ...
                 <div>
                    <h3 className="text-xl font-semibold mb-4 text-center">Restaurada</h3>
                     <div className="w-full aspect-square bg-gray-700 rounded-lg shadow-lg p-2">
                        ...
                        {resultImage && <img src={resultImage} alt="Restored photo" className="w-full h-full object-contain" />}
                    </div>
                </div>
            </div>
        </div>

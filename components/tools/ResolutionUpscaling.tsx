...
interface ResolutionUpscalingProps {
    tool: Tool;
    onBack: () => void;
}

const ResolutionUpscaling: React.FC<ResolutionUpscalingProps> = ({ tool, onBack }) => {
    ...
    const handleGenerate = useCallback(async () => {
        ...
        setResultImage(null);
        try {
            const prompt = "Recreate this image at a much higher resolution with more detail. Enhance the quality and sharpness, making it suitable for high-quality printing. This is an AI upscaling task.";
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="mt-6 mb-8 p-4 bg-gray-800 border border-gray-700 rounded-lg text-center">
                <p className="text-sm text-gray-300">
                    <span className="font-semibold text-purple-400">¿Cómo funciona?</span> Sube una imagen de baja resolución o pixelada. La IA aumentará su tamaño y calidad, añadiendo detalles para que se vea nítida y profesional (upscaling).
                </p>
            </div>
            ...
            <div className="bg-gray-800/50 p-6 rounded-lg text-center">
                <div className="w-full p-4 border-2 border-dashed border-gray-600 rounded-lg">
                    ...
                    <label htmlFor="image-upload" className={`cursor-pointer ${isLoading ? 'cursor-not-allowed' : ''}`}>
                        {imagePreview ? (
                            <p>Imagen seleccionada. ¡Lista para mejorar!</p>
                        ) : (
                            <p>Click to upload a low-resolution image</p>
                        )}
                    </label>
                </div>
                 <button
                ...
                >
                    {isLoading && <Spinner />}
                    {isLoading ? 'Mejorando Resolución...' : 'Mejorar Resolución'}
                </button>
            </div>
            ...
            <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                <div>
                 ...
                 <div>
                    <h3 className="text-xl font-semibold mb-4 text-center">Mejorada</h3>
                     <div className="w-full aspect-square bg-gray-700 rounded-lg shadow-lg p-2">
                        ...
                        {resultImage && <img src={resultImage} alt="Upscaled image" className="w-full h-full object-contain" />}
                    </div>
                </div>
            </div>
        </div>
...
export default ResolutionUpscaling;
